...
* For the `titles_basics` table
  ...
  * use two different variables to group and explore how `runtimeMinutes` varies for these different groups. Produce appropriate summaries. 
  For most genres, the runtime drops as runtime increases. This pattern is consistent across all genres, including Action, Comedy, Drama, Documentary, Animation, and others.
For animation and short, these genres show the highest density at the shortest runtimes, which aligns with expectations since animated and short films are typically brief.
  ...
  * Graph the conditional distributions of release year based on the previous results. Comment on any trends you observe.
  Comedy,drama, and action have consistenly remained among the top 3 most frequent genres across all decades. Newer genres like Animation have gained significant market share in recent years. 
...
Profiling reveals that the separate_rows() function is a major bottleneck due to significant row expansion.
The inner_join() with title_ratings adds further computational overhead.
Possible improvements could include using data.table::tstrsplit() to avoid list expansion, or pre-filtering data.
Overall, the current pipeline is acceptable for this dataset size but would need optimization for full IMDb data.
...
Using mclapply reduced the wall time by approximately 38%.
Parallelization benefits this task because it is embarrassingly parallel — each string can be split independently.
...
The str_count() + 1 approach was approximately 7× faster (median 138 ms vs 960 ms) with similar memory usage.
Thus, f2_count is recommended for production pipelines, especially for very large datasets.
