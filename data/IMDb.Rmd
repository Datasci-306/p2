...
title_ratings <- title_ratings %>%
  ...
  filter(!is.na(rating_group))  # Remove NA values
...
Find names 
```{r}
# Ensure primaryProfession and primaryName are character
names_basics <- names_basics %>%
  mutate(
    primaryProfession = as.character(primaryProfession),
    primaryName = as.character(primaryName)
  )
...
# Filter for actors with matching first names
actors_tom <- names_basics %>%
  filter(
    tolower(first_name) %in% c("tom", "thomas", "thom", "tomas"),
    grepl("actor", primaryProfession, ignore.case = TRUE)
  )

# Count
num_actors_tom <- nrow(actors_tom)
print(paste("Number of actors named Tom/Thomas/Thom/Tomas:", num_actors_tom))
```
...
is_alliteration <- function(title) {
  ...
  words <- gsub("[^A-Za-z]", "", words) # Remove punctuation
  words <- words[words != ""]           # Remove empty strings
  if(length(words) < 2) return(FALSE)   # Need at least 2 words
  first_letters <- toupper(substr(words, 1, 1))
  length(unique(first_letters)) == 1
}

# Apply to all titles
alliteration_mask <- sapply(title_basics$primaryTitle, is_alliteration)
num_alliteration_titles <- sum(alliteration_mask, na.rm = TRUE)
print(paste("Number of alliterative titles:", num_alliteration_titles))
```
